...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_no_end) { described_class.new(5, '4') }
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(game_no_end).not_to be_game_over
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        letter_2 = 'e'
        valid_input = '8'
        allow(game_loop).to receive(:player_input).and_return(letter, letter_2, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_two_to_three) { described_class.new(5, '5', 3) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_two_to_three.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:game_over_four) { described_class.new(5, '5', 4) }      
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        pity_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_over_four.final_message }.to output(pity_phrase).to_stdout
      end
    end
  end
end
