...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        average = game_guessing.make_guess
  ...
  describe '#game_over?' do
    let(:number_game_over) { double('number_game_over', value: 8) }
    context 'when guess and random_number are equal' do
      subject(:game_over_check) { described_class.new(0, 9, number_game_over, 8) }
      ...
      it 'is game over' do
        expect(game_over_check).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_over_check) { described_class.new(0, 9, number_game_over, 9) }
      it 'is not game over' do
        expect(game_over_check).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'does not update max' do
        max_pre_update = low_guess_game.max
        low_guess_game.update_range
        expect(low_guess_game.max).to eq(max_pre_update)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        min_pre_update = high_guess_game.min
        high_guess_game.update_range
        expect(high_guess_game.min).to eq(min_pre_update)
      end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:final_guess_game) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        final_guess_game.update_range
        expect(final_guess_game.min).to eq(final_guess_game.max)
      end

      it 'does not update max' do
        max_pre_update = final_guess_game.max
        final_guess_game.update_range
        expect(final_guess_game.max).to eq(max_pre_update)
      end
    end
  end
end
