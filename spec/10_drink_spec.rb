...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:whisky_coke) { described_class.new('Whisky') }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(whisky_coke.type).to eq('Whisky')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(whisky_coke.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:orange_juice) { described_class.new('Orange Juice') }
      # remove the 'x' before running this test
      it 'is full' do
        expect(orange_juice).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:melon_juice) { described_class.new('Melon Juice', 8) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(melon_juice).not_to be_full
      end
    end
  end
end
